...
static sys_sem_t player_semaid;
static sys_sem_attr_t player_sema_attr;
...
if(it_have_samples==0) {it_have_samples=1;sysSemPost(player_semaid, 1);return;}
...
static s32 SndBufStart(MODSNDBUF *sndbuf)
{
    ...
    if (sysThreadCreate(&h_player, (void *) player, (u64) 0, 900, STACKSIZE,
...
static void SndBufStop()
{
    ...
    sysSemPost(player_semaid, 1);

	sysThreadJoin(h_player, NULL);
...
void MODPlay_Init(MODPlay *mod)
{
	...
	memset(&player_sema_attr, 0, sizeof(sys_sem_attr_t));

    player_sema_attr.attr_protocol = 0x2;
    player_sema_attr.attr_pshared  = 0x2 << 8;

    sysSemCreate(&player_semaid, &player_sema_attr, 0, 1);
